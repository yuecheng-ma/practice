...
class MultiLayerPerceptron(torch.nn.Module):
    def __init__(self, num_inputs, num_outputs, num_hiddens=256):
        super().__init__()
        self._input_layer = nn.Linear(num_inputs, num_hiddens)
        self._relu = nn.ReLU()
        self._output_layer = nn.Linear(num_hiddens, num_outputs)

    def forward(self, inputs):
        inputs = inputs.view(inputs.shape[0], -1)
        h = self._relu(self._input_layer(inputs))
...
if __name__ == "__main__":
    ...
    model = MultiLayerPerceptron(28 * 28, 10)
